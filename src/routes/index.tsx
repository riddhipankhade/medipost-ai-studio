  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <section className="relative overflow-hidden">
        <div className="absolute inset-0 -z-10" style={{ background: "radial-gradient(60% 60% at 50% 0%, oklch(0.93 0.06 200 / 0.7), transparent 70%)" }} />
        <div className="max-w-6xl mx-auto px-6 py-20 md:py-28 text-center">
          <Badge variant="secondary" className="mb-5 gap-1.5"><Sparkles className="h-3 w-3 text-[color:var(--teal)]" /> Built for healthcare professionals</Badge>
          <h1 className="text-4xl md:text-6xl font-semibold tracking-tight max-w-3xl mx-auto">
            AI content your <span className="text-[color:var(--teal)]">patients actually read.</span>
          </h1>
          ...
          <div className="rounded-2xl border border-border bg-card overflow-hidden" style={{ boxShadow: "var(--shadow-soft)" }}>
            ...
            <div className="grid md:grid-cols-2 gap-0">
              ...
              <div className="p-6 bg-accent/30">
                ...
                <pre className="whitespace-pre-wrap font-sans text-sm leading-relaxed">{`✨ A brighter smile starts with small daily habits!\n\n1. Brush twice a day (2 min each!)\n2. Floss before bed\n3. Swap soda for water\n4. Visit your dentist every 6 months\n\n#DentalCare #HealthySmile`}</pre>
              </div>
            </div>
          </div>
        </div>
      ...
      <section id="features" className="py-20 bg-[oklch(0.985_0.01_220)]">
        <div className="max-w-6xl mx-auto px-6">
          <div className="text-center max-w-2xl mx-auto mb-12">
            <h2 className="text-3xl md:text-4xl font-semibold tracking-tight">Everything a modern practice needs</h2>
            <p className="text-muted-foreground mt-3">From social posts to patient handouts — built around the way doctors actually work.</p>
          </div>
          <div className="grid md:grid-cols-3 gap-5">
            <Feature icon={Instagram} title="Instagram posts" desc="Engaging, on-brand captions with the right hashtags for your specialty." />
            <Feature icon={FileText} title="Patient education" desc="Clear, friendly handouts your patients can take home and understand." />
            <Feature icon={BookOpen} title="Blog articles" desc="Long-form, SEO-friendly articles to grow your clinic's online presence." />
            <Feature icon={Stethoscope} title="Specialty-aware" desc="Dental, derma, cardio, GP — content adapts to your field automatically." />
            <Feature icon={Clock} title="30-second drafts" desc="From idea to publishable draft faster than writing the first line yourself." />
            <Feature icon={Shield} title="Medically grounded" desc="Built-in guardrails to keep content safe, factual and patient-appropriate." />
          </div>
        </div>
      </section>
      ...
      <section id="how" className="py-20">
        <div className="max-w-5xl mx-auto px-6 grid md:grid-cols-3 gap-8 text-center">
          {[
            ...
            <div key={s.n}>
              <div className="mx-auto h-10 w-10 rounded-full grid place-items-center text-white font-semibold" style={{ background: "var(--brand-gradient)" }}>{s.n}</div>
      ...
      <section id="pricing" className="py-20 bg-[oklch(0.985_0.01_220)]">
        <div className="max-w-6xl mx-auto px-6">
          <div className="text-center max-w-2xl mx-auto mb-12">
            <h2 className="text-3xl md:text-4xl font-semibold tracking-tight">Simple, doctor-friendly pricing</h2>
            <p className="text-muted-foreground mt-3">Start small. Upgrade when your practice grows.</p>
          </div>
          <div className="grid md:grid-cols-3 gap-5">
            <Plan name="Starter" price="₹499" gens="50 generations / month" features={["All content types", "Copy & save", "Email support"]} />
            <Plan name="Pro" price="₹1,999" gens="300 generations / month" highlight features={["Everything in Starter", "Priority generation", "Content history & search", "Brand tone presets"]} />
            <Plan name="Clinic" price="₹6,999" gens="Unlimited generations" features={["Everything in Pro", "Up to 10 doctor seats", "Team library", "Dedicated success manager"]} />
          </div>
        </div>
      </section>
      ...
      <section className="py-20">
        <div className="max-w-3xl mx-auto px-6 text-center">
          ...
          <p className="text-muted-foreground mt-3">Join 1,200+ doctors creating with Medipost AI.</p>
...
function Plan({ name, price, gens, features, highlight }: { name: string; price: string; gens: string; features: string[]; highlight?: boolean }) {
  return (
    <div className={`relative rounded-2xl border bg-card p-6 ${highlight ? "border-[color:var(--teal)] ring-2 ring-[color:var(--teal)]/40" : "border-border"}`}>
      {highlight && <Badge className="absolute -top-3 left-5 bg-[color:var(--teal)] text-white hover:bg-[color:var(--teal)]">Most popular</Badge>}
      <p className="text-sm font-medium text-muted-foreground">{name}</p>
      <div className="flex items-baseline gap-1 mt-2">
        <span className="text-3xl font-semibold">{price}</span>
        <span className="text-sm text-muted-foreground">/month</span>
      </div>
      <p className="text-sm text-[color:var(--teal)] mt-1">{gens}</p>
      <ul className="space-y-2 text-sm mt-5">
        {features.map((f) => <li key={f} className="flex gap-2"><Check className="h-4 w-4 text-[color:var(--teal)] mt-0.5" />{f}</li>)}
      </ul>
      <Button asChild className="w-full mt-6" variant={highlight ? "default" : "outline"}>
